...
変数
- 変数名の先頭にハイフン、数字は使えない
- 代入は `変数名=値`　とする。スペースを入れないこと
- `file='Document files'`　値にスペースを含む場合はクォート`''`で囲む
- 変数の参照は`$`を使って`$file` とする
- 値を設定せずに`$file`を呼び出すと空文字列が返る。エラーにはならない
...
- 配列の呼び出しは{}でくくる
  - `echo ${a[0]}`
- 日付呼び出し。日本語になる `echo $(date +%Y-%m-%d\ %H:%M:%S)` -> 2021-12-17 08:41:07
